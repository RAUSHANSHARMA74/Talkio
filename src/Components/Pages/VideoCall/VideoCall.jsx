...
import { FaVideo, FaMicrophone, FaCircle, FaUserPlus, FaEllipsisV } from 'react-icons/fa';

export default function VideoCall() {
    // Dummy list of participants, including the user
    const participants = [
        { id: 1, name: 'You' },
        { id: 2, name: 'John Doe' },
    ...
    // State to track the selected participant for enlarged view
    const [selectedParticipant, setSelectedParticipant] = useState(null);

    // Function to handle participant click
    const handleParticipantClick = (participant) => {
        // Toggle the selected participant
        if (selectedParticipant?.id === participant.id) {
            setSelectedParticipant(null); // Deselect if clicked again
        } else {
